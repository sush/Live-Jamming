    ...
    <tr id="category_<?php echo $category['id']; ?>"<?php if ($counter % 2) echo ' class="altRow"'; ?>>
        ...
        <td>
            ...
            if (!empty($category['LastTopic'])) {
                ...
                <em><?php echo $time->relativeTime($lastTime, array('userOffset' => $cupcake->timezone())); ?></em> <span class="gray"><?php __d('forum', 'by'); ?> <?php echo $html->link($category['LastUser']['login'], array('controller' => 'users', 'action' => 'profile', $category['lastUser_id'])); ?></span>
            <?php } else {
...
<div id="forumStats">
    ...
    <?php // Newest user
	if (!empty($newestUser)) { ?>
    <strong><?php __d('forum', 'Newest User'); ?></strong>: <?php echo $html->link($newestUser['User']['login'], array('controller' => 'users', 'action' => 'profile', $newestUser['User']['id'])); ?>
   	<?php } ?>
	...
	if (!empty($whosOnline)) {
		...
		foreach ($whosOnline as $user) {
			$online[] = $html->link($user['User']['login'], array('controller' => 'users', 'action' => 'profile', $user['User']['id']));
		} ?>
