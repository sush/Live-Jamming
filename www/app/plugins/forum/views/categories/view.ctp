    ...
    <tr id="category_<?php echo $subCat['id']; ?>"<?php if ($counter % 2) echo ' class="altRow"'; ?>>
        ...
        <td>
            ...
            if (!empty($subCat['LastTopic'])) {
                ...
                <em><?php echo $time->relativeTime($lastTime, array('userOffset' => $cupcake->timezone())); ?></em> <span class="gray"><?php __d('forum', 'by'); ?> <?php echo $html->link($subCat['LastUser']['login'], array('controller' => 'users', 'action' => 'profile', $subCat['lastUser_id'])); ?></span>
            <?php } else {
...
<div id="topicWrap">
    ...
    <tr>
        ...
        <th><?php echo $paginator->sort(__d('forum', 'Author', true), 'User.login'); ?></th>
	...
	<?php foreach ($stickies as $topic) {
   	...
   	<tr<?php if ($counter % 2) echo ' class="altRow"'; ?>>
        ...
        <td class="ac"><?php echo $html->link($topic['User']['login'], array('controller' => 'users', 'action' => 'profile', $topic['User']['id'])); ?></td>
        ...
        <td>
            ...
            if (!empty($topic['LastPost'])) {
                ...
                <span class="gray"><?php __d('forum', 'by'); ?> <?php echo $html->link($topic['LastUser']['login'], array('controller' => 'users', 'action' => 'profile', $topic['Topic']['lastUser_id'])); ?></span>
...
<div id="categoryStats">
	...
	if (!empty($category['Moderator'])) {
		foreach ($category['Moderator'] as $mod) {
			$moderators[] = $html->link($mod['User']['login'], array('controller' => 'users', 'action' => 'profile', $mod['User']['id'])); 
		}	
	} ?>
