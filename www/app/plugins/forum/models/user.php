...
class User extends ForumAppModel {
	...
	public $validate = array(
		'login' => array(
			'isUnique' => array(
				'rule' => 'isUnique',
				'message' => 'That login has already been taken',
				'on' => 'create'
			),
			'notEmpty' => array(
				'rule' => 'notEmpty',
		...
		$user = $this->find('first', array(
			'conditions' => array(
				'OR' => array(
					...
					array('User.login' => $data['User']['login'])	
				)
			)
		));
		
		if (empty($user)) {
			$this->invalidate('login', 'No user was found with either of those credentials');
			return false;
		}
	...
	public function beforeValidate() {
		...
		if ($action == 'login') {
			unset($this->validate['login']['isUnique']);
