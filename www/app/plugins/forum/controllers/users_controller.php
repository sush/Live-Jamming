...
class UsersController extends ForumAppController {
	...
	public function logout() {
		//$this->Session->delete('Forum');
		//$this->redirect($this->Auth->loginRedirect = array('plugin' => null, 'controller' => 'users', 'action' => 'logout'));
	}
	...
	public function listing() {
		if (!empty($this->data)) {
			if (!empty($this->data['User']['login'])) {
				$this->paginate['User']['conditions']['User.login LIKE'] = '%'. $this->data['User']['login'] .'%';
			}
		}
	...
	public function profile($id) {
		...
		$this->Toolbar->pageTitle(__d('forum', 'User Profile', true), $user['User']['login']);
		$this->set('user', $user);
	}
	...
	public function report($id) {
		...
		$user = $this->User->get($id, array('id', 'login'));
		$user_id = $this->Auth->user('id');
	...
	public function signup() {
		if (!empty($this->data)) {
			...
			if ($this->User->validates()) {
				$this->data['User']['login'] = strip_tags($this->data['User']['login']);
				$this->data['User']['password'] = $this->Auth->password($this->data['User']['newPassword']);
				
				if ($this->User->save($this->data, false, array('login', 'email', 'password'))) {
					...
					$message .= __d('forum', 'Login', true) .": ". $this->data['User']['login'] ."\n";
	...
	public function admin_edit($id) {
		...
		if (!empty($this->data)) {
			...
			if ($this->User->save($this->data, true, array('login', 'email', $this->User->columnMap['totalPosts'], $this->User->columnMap['totalTopics']))) {
	...
	public function admin_reset($id) {
		...
		if (!empty($user)) {
			...
			$this->Session->setFlash(sprintf(__d('forum', 'The password for %s has been reset!', true), '<strong>'. $user['User']['login'] .'</strong>'));
		}
	...
	public function admin_delete($id) {
		...
		if (!empty($this->data['User']['delete'])) {
			...
			$this->Session->setFlash(sprintf(__d('forum', 'The user %s and all of their associations have been deleted!', true), '<strong>'. $user['User']['login'] .'</strong>'));
			$this->redirect(array('controller' => 'users', 'action' => 'index', 'admin' => true));
		}
