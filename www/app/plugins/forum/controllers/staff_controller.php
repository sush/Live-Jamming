...
class StaffController extends ForumAppController {
	...
	public function admin_delete_access($id) {
		$access = $this->Access->get($id, false, array('User.login'));
		$this->Toolbar->verifyAccess(array('exists' => $access));
		
		if (!empty($access)) {
			$this->Access->delete($id, true);
			$this->Session->setFlash(sprintf(__d('forum', 'The access levels for %s have been succesfully removed.', true), '<strong>'. $access['User']['login'] .'</strong>'));
		}
	...
	public function admin_delete_moderator($id) {
		$mod = $this->Moderator->get($id, false, array('User.login'));
		$this->Toolbar->verifyAccess(array('exists' => $mod));
