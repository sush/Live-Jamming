...
class SearchController extends ForumAppController {
	...
	public $paginate = array( 
		'Topic' => array(
			...
			'contain' => array('ForumCategory.title', 'ForumCategory.slug', 'User.id', 'User.login', 'LastPost.created', 'LastUser.login', 'Poll.id', 'FirstPost.content')
		)
	); 
	...
	public function index($type = '') {
		...
		if (!empty($this->data)) {
			...
			if (!empty($this->data['Topic']['byUser'])) {
				$this->paginate['Topic']['conditions']['User.login LIKE'] = '%'. $this->data['Topic']['byUser'] .'%';
			}
