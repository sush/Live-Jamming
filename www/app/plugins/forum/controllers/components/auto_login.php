...
class AutoLoginComponent extends Object {
	...
	public function startup(&$Controller) {
		...
		if ($cookie['hash'] != $Controller->Auth->password($cookie[$Controller->Auth->fields['login']] . $cookie['time'])) {
			$this->delete();
			return;
		}
	...
	public function beforeRedirect(&$Controller) {
		...
		if ($Controller->plugin == $plugin && $Controller->name == $controller) {
			...
			switch ($Controller->action) {
				case $loginAction:
					$login = $data[$Controller->Auth->userModel][$Controller->Auth->fields['login']];
					...
					if (!empty($login) && !empty($password) && $autoLogin == 1) {
						$this->save($login, $password, $Controller);
					} else if ($autoLogin == 0) {
						$this->delete();
					}
				break;
	 ...
	 * @return void
	 */
	public function save($login, $password, $Controller) {
		$time = time();
		$cookie = array();
		$cookie[$Controller->Auth->fields['login']] = $login;
		$cookie[$Controller->Auth->fields['password']] = $password; // Already hashed from auth
		$cookie['hash'] = $Controller->Auth->password($login . $time);
		$cookie['time'] = $time;
