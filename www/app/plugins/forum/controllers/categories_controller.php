...
class CategoriesController extends ForumAppController {
	...
	public $paginate = array(  
		'Topic' => array(
			...
			'contain' => array('User.id', 'User.login', 'LastPost.created', 'LastUser.login', 'Poll.id')
		)
	);
	...
	public function feed($slug) {
		if ($this->RequestHandler->isRss()) {
			...
			$this->paginate['Topic']['limit'] = $this->Toolbar->settings['topics_per_page'];
			$this->paginate['Topic']['conditions'] = array('Topic.forum_category_id' => $category['ForumCategory']['id']);
			$this->paginate['Topic']['contain'] = array('User.id', 'User.login', 'LastPost.created', 'FirstPost.content');
